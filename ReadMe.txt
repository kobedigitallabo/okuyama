...
[[リリース Ver 0.8.6 - (2011/XX/XX)]]
  ...
  ■遅延レプリケーション機能追加
    DataNodeのレプリケーション先を設定している場合に、レプリケーションのデータを転送後、即成功とする。
    書き込み処理が高速化される反面、ノードダウン時にデータ整合性が失われる可能性が高い

  ■Tagを内部で管理する際のデータ構造を変更し、処理速度を向上

  ■共有データファイルに対するアクセスを並列で行うように変更
