...
========================================================================================================
[New - 不具合対応]
[[リリース Ver 0.9.6.3 - (2015/11/02)]]
バグを発見したため、急遽リリースを行います。
■バグ内容
　・ディスクモード時に使用できるはずのVacuum処理が常に使用不可となっていた。
　・Vacuum処理後、削除されていない有効なデータも取得できなくなる。

■修正
　◎ Vacuum関連の設定をDataNodeの起動引数から行えるように修正。追加された起動引数は以下の通り。
...
　　例) Key値の数とファイルの行数の差が1000行を超えるとVacuumを行う候補とする設定。
　　　　-vac true -vacsl 1000

　　例) Key値の数とファイルの行数の差が1000行を超えると強制的にVacuumを行う候補とする設定。
　　　　-vac true -vacscl 1000
　　　　
　◎ Vacuum処理により削除されていない有効なデータが取得できなくならないように修正。

========================================================================================================
[New - 不具合対応]
[[リリース Ver 0.9.6.2 - (2015/10/15)]]
バグを発見したため、急遽リリースを行います。
■バグ内容
　OkuyamaClientでは指定最大サイズを超えるデータが保存されないように保存前にvalueのチェックをいている。
　そのValueのチェックにおいてsetObjectValueにおいてValueのサイズをBase64する前のサイズでチェックしていた。
ValueはBase64エンコード後にMasterNodeえ付与される管理情報と共に保存される。そのため、DataNodeに指定された
最大サイズよりサイズが大きいValueの保存リクエストが投げられ、Valueがディスクのときにエラーとなる。

■修正
　setObjectValue時、OKuyamaClientでBase64エンコード後のサイズにMasterNodeで付与される管理情報分のサイズを
追加したサイズでValueのサイズチェックを行う。
 
========================================================================================================
[New - 不具合対応]
[[リリース Ver 0.9.6.1 - (2014/09/02)]]
バグを発見したため、急遽リリースを行います。
■バグ内容
　DataNodeの起動引数に「-csf false」を指定した状態で管理ポートでのコマンド「allsize」を実行すると
例外ArrayIndexOutOfBoundsExceptionが発生して、allsizeコマンドが失敗する。

■修正
　-csf falseが指定されているDataNodeがある状態で「allsize」を実行すると、all=-1を表示するように修正。
